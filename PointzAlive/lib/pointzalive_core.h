...
#include "PA_GameIncludes.h"
#include "PA_Headers.h"
#include "PA_Game_Selection.h"
...
class pointzalive_core {

private:
    int selection;
    Space *myMap;
    vector<int> nColors;
    template_game* mygame;

    std::function<bool(int &, int &)> hasSelectionChanged = [](int &a, int &b){return a != b;};

public:
    pointzalive_core(Space *map): selection(NOGAME), myMap(map){
        ...
        if(ImGui::BeginMenuBar() ) {
            if (ImGui::BeginMenu("Select game")) {
                ...
                if (ImGui::MenuItem("NoGame")) {
                    ...
                    selection = BALLPIT;
                    onParameterChanged(selection, tempSelection, myMap, hasSelectionChanged, nogame::init);
                }else if (ImGui::MenuItem("Life pointzalive_core")) {
                    selection = LIFE_GAME;
                    onParameterChanged(selection, tempSelection, myMap, hasSelectionChanged, nogame::init);
